...
# 3D dynamic program event planner file

# Minor changes, eg extra if statement
...
def main():
    running = True
    constraints = ["Cost", "Time", "Both"]
    inputs = ["input_small.txt", "input_medium.txt","input_large.txt", "input_1000.txt"]
    
    print("\nWelcome to the Event Planner, here you can run a brute force and dynamic algorithm to find the best schedule within a given constraint!")
    ...
    while running:
        ...
        print("Pick a constraint: Cost, Time, or Both")
        looping = True
        while looping:
