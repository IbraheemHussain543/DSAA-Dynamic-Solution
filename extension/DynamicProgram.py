...
def create_matrix():
    # Support either single-constraint 2D DP or two-constraint 3D DP
    if constraint[0] == "Both":
        time_allowed,cost_allowed = constraint[1]
        # matrix dimensions: (num_acts+1) * (time_allowed+1) * (cost_allowed+1)
        return [ [ [0] * (cost_allowed + 1) for _ in range(time_allowed + 1) ] for _ in range(num_acts + 1) ]
    else:
        matrix = []
...
def fill_matrix(matrix):
    ...
    rows = len(keys) + 1
    # If optimizing by both time and cost, use a 3D table
    if constraint[0] == "Both":
        time_allowed,cost_allowed = constraint[1]
        for row in range(1, rows):
            curr_act = activities[keys[row - 1]]
            act_time = int(curr_act[0])
            act_cost = int(curr_act[1])
            value = int(curr_act[2])
            for t in range(0, time_allowed + 1):
                for c in range(0, cost_allowed + 1):
                    skip = matrix[row - 1][t][c]
                    take = 0
                    if t >= act_time and c >= act_cost:
...
def extract_solution(matrix):
    ...
    keys = list(activities.keys())
    rows = len(keys) + 1
    # If both constraints used, trace back through 3D table
    if constraint[0] == "Both":
        time_allowed, cost_allowed = constraint[1]
        curr_t = time_allowed
        curr_c = cost_allowed
        for curr_row in range(rows - 1, 0, -1):
            curr_joy = matrix[curr_row][curr_t][curr_c]
            above_joy = matrix[curr_row - 1][curr_t][curr_c]
            if curr_joy != above_joy:
                optimal_schedule.append(keys[curr_row - 1])
                # subtract the item's time and cost
                curr_t -= int(activities[keys[curr_row - 1]][0])
                curr_c -= int(activities[keys[curr_row - 1]][1])
    else:
        cols = len(matrix[0])
